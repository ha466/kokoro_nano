...
class Decoder(nn.Module):
    def __init__(self, dim_in, style_dim, dim_out, 
                 ...
                 # These two widths were hardcoded at 1024/512, so no config could shrink the
                 # decoder -- 53.28M of Kokoro's 82M. The defaults reproduce the stock model
                 # exactly, so existing checkpoints load unchanged.
        ...
        # NOTE: this consumes `asr` (dim_in channels), NOT the decode-block width. In the
        # stock model dim_in == out_channels == 512, so the two are indistinguishable
        # until they are shrunk independently.
